...
const NAMESPACE = 'kwc-calendar';
const BUILD = /* kwc-calendar */ { hydratedSelectorName: "hydrated", slotRelocation: true, updatable: true};
...
class KwcDate {
    ...
    add(diff) {
        var _a, _b, _c, _d, _e;
        const result = {
            years: this.year + ((_a = diff.years) !== null && _a !== void 0 ? _a : 0),
            month: this.month + ((_b = diff.months) !== null && _b !== void 0 ? _b : 0),
            day: this.day + ((_c = diff.days) !== null && _c !== void 0 ? _c : 0),
            hours: this.hours + ((_d = diff.hours) !== null && _d !== void 0 ? _d : 0),
            minutes: this.minutes + ((_e = diff.minutes) !== null && _e !== void 0 ? _e : 0),
        };
...
class KwcDateLocalization {
    constructor(locale) {
        this.locale = locale !== null && locale !== void 0 ? locale : (navigator.languages && navigator.languages.length ? navigator.languages[0] : navigator.language);
    }
    get firstDayOfWeek() {
...
const KwcCalendarMonthViewHeader = (props) => {
    return (hAsync("div", { part: PARTS.CALENDAR_YM_WRAPPER, class: "nav-ym-wrapper" }, hAsync("button", { part: parts([PARTS.CALENDAR_NAV, PARTS.CALENDAR_NAV_MONTH]), class: "nav-month", onClick: () => props.onSwitchView('year') }, props.localization.getMonth(props.viewState)), hAsync("button", { part: parts([PARTS.CALENDAR_NAV, PARTS.CALENDAR_NAV_YEAR]), class: "nav-year", onClick: () => props.onSwitchView('decade') }, props.localization.getYear(props.viewState))));
};
const KwcCalendarMonthViewBody = (props) => {
    return (hAsync("div", { part: parts(PARTS.CALENDAR_MONTH_VIEW), class: "month-view" }, props.localization.getWeekdays().map(x => (hAsync("span", { part: PARTS.CALENDAR_WEEK_DAY, class: "weekday", key: x }, x))), KwcViewBuilder.create(props.localization)
        .forMonth(props.viewState)
        .padLeft()
        .padRight()
        .extend({ weeks: 6 })
        .done()
        .map(x => (hAsync("button", { part: parts({
        ...
        }, key: `${x.year}-${x.month}-${x.day}`, onClick: () => props.onDateSelected(x) }, props.localization.getDay(x))))));
};

const KwcCalendarYearViewHeader = (props) => {
    return (hAsync("div", { part: PARTS.CALENDAR_YM_WRAPPER, class: "nav-ym-wrapper" }, hAsync("button", { part: parts([PARTS.CALENDAR_NAV, PARTS.CALENDAR_NAV_YEAR]), class: "nav-year", onClick: () => props.onSwitchView('decade') }, props.localization.getYear(props.viewState))));
};
const KwcCalendarYearViewBody = (props) => {
    return (hAsync("div", { part: parts(PARTS.CALENDAR_YEAR_VIEW), class: "year-view" }, KwcViewBuilder.create(props.localization)
        ...
        }, key: `${x.year}-${x.month}-${x.day}`, onClick: () => props.onSwitchView('month', x) }, props.localization.getShortMonth(x))))));
};

const KwcCalendarDecadeViewHeader = (props) => {
    return (hAsync("div", { part: PARTS.CALENDAR_YM_WRAPPER, class: "nav-ym-wrapper" }, hAsync("span", { class: "nav-decade" }, props.localization.getDecade(props.viewState))));
};
const KwcCalendarDecadeViewBody = (props) => {
    return (hAsync("div", { part: parts(PARTS.CALENDAR_DECADE_VIEW), class: "decade-view" }, KwcViewBuilder.create(props.localization)
        ...
        }, key: `${x.year}-${x.month}-${x.day}`, onClick: () => props.onSwitchView('year', x) }, props.localization.getYear(x))))));
};
...
const kwcCalendarCss = ":host *{box-sizing:border-box}:host{box-sizing:border-box;position:relative;display:inline-flex;flex-direction:column;background-color:white;padding:0.5rem;border:1px solid black;user-select:none}.header{width:100%;display:flex;justify-content:space-between;align-items:center;gap:4px}.header .nav-arrow{display:flex;align-items:center;justify-content:center;background-color:transparent;padding:0;border:1px solid transparent;font-size:1.4rem}.header .nav-arrow:hover{background-color:#bed6fe;border:1px solid black}.nav-ym-wrapper{flex:1 0 auto;display:flex;justify-content:center;gap:2px}.header .nav-month,.header .nav-year{background-color:transparent;border:1px solid transparent}.header .nav-month:hover,.header .nav-year:hover{background-color:#bed6fe;border:1px solid black}.header .nav-decade{font-size:0.85rem;padding:0 16px}.month-view{display:inline-grid;grid-template-columns:repeat(7, 1fr);justify-items:center}.year-view{display:inline-grid;grid-template-columns:repeat(3, 1fr);justify-items:center}.decade-view{display:inline-grid;grid-template-columns:repeat(2, 1fr);justify-items:center}.weekday{height:1.6rem;justify-self:center}.btn-day{width:1.7rem;font-size:0.8rem;aspect-ratio:1/1;border:none;background-color:transparent;padding:0}.btn-month,.btn-year{padding:0.2rem;border:1px solid transparent;background-color:transparent}.btn-day:hover,.btn-month:hover,.btn-year:hover{background-color:#bed6fe;border:1px solid black}.btn-day.selected,.btn-month.selected,.btn-year.selected{background-color:#2876fc;color:#ffffff;border:2px solid black}.btn-day.current:not(.selected),.btn-month.current:not(.selected),.btn-year.current:not(.selected){border:1px solid black}.btn-day.out-of-month{color:#767676}";
...
class KwcCalendar {
    ...
    locale = null;
    current = KwcDate.now();
    selected = KwcDate.now();
    viewState = KwcDate.now();
    valueChanged;
    localization;
    componentWillLoad() {
    ...
    view = 'month';
    switchView(view, kwcDate) {
        this.view = view;
        if (kwcDate)
            this.viewState.setDate(kwcDate);
    }
    render() {
        return (hAsync(Host, { key: 'a3cd66ef17b6e72c31c71f32217c4fbe7aa825a5', part: PARTS.CALENDAR }, hAsync("div", { key: '5b85bfbc8f0d78540dd54044d5464f1a1647a453', part: PARTS.CALENDAR_HEADER, class: "header" }, hAsync("button", { key: '298d82b76d91b70d4fcb88b93ad0b85471076006', part: parts([PARTS.CALENDAR_NAV, PARTS.CALENDAR_NAV_ARROW, PARTS.CALENDAR_NAV_ARROW__PREV]), class: "nav-arrow", onClick: () => this.moveToPrev() }, hAsync("slot", { key: '380543a75538a881b3354e2f618507cc6e72688f', name: "arrow-left" }, "\uD83E\uDC60")), views.header[this.view]({
            localization: this.localization,
            viewState: this.viewState,
            onSwitchView: view => this.switchView(view),
        }), hAsync("button", { key: '64e7b124f8364613e34d6ab435dffa03bf1d71b6', part: parts([PARTS.CALENDAR_NAV, PARTS.CALENDAR_NAV_ARROW, PARTS.CALENDAR_NAV_ARROW__NEXT]), class: "nav-arrow", onClick: () => this.moveToNext() }, hAsync("slot", { key: 'e91a91974a4d1d4775af03dffe98d257be8078ad', name: "arrow-right" }, "\uD83E\uDC62"))), views.body[this.view]({
            localization: this.localization,
            current: this.current,
            selected: this.selected,
            viewState: this.viewState,
            onDateSelected: (kwcDate) => this.setSelectedDate(kwcDate),
    ...
    static get style() { return kwcCalendarCss; }
